...
if __name__ == '__main__':
    ...
    for cls_name, clf in zip(names, classifiers):
        ...
        predict_cover = clf.predict(X_test)
        # print(subject)
        print(y_test)
        print(predict_cover)


        clf.fit(X_train.iloc[:, 0: 148], y_train)
        predict_result = clf.predict(X_test.iloc[:, 0: 148])
        result = []
        for i in range(len(subject)):
            if not np.array_equal(y_test.values[i], predict_result[i]):
                temp = []
                print(i)
                ...
                # print(df[i: i+1])
                try:
                    print(df['Unnamed: 1'][y_test.index[i]])
                except:
                    print('origin')
                print(y_test.values[i])
                print(predict_result[i])
                print(predict_cover[i])
                temp.append(subject[i])
                try:
                    temp.append(df['Unnamed: 1'][y_test.index[i]])
                except:
                    temp.append("origin")
                temp.extend(y_test.values[i])
                temp.extend(predict_result[i])
                temp.extend(predict_cover[i])
                result.append(temp[:])
        # print(result)
        if not os.path.isfile('{}/Predict_dif.xlsx'.format(classifier_dir)):
            wb = Workbook()
        else:
            wb = load_workbook('{}/Predict_dif.xlsx'.format(classifier_dir))
        ws = wb.create_sheet(cls_name)
        for row in result:
            ws.append(row)
        wb.save('{}/Predict_dif.xlsx'.format(classifier_dir))

        # print(clf.predict(X_test).shape, y_test.shape, subject.T.shape)
        '''result = np.column_stack((subject.T, y_test, predict_cover))
    ...
    '''print(X_train.shape)
    # print(type(X_train))
    X_train = X_train.iloc[:, 0: 148]
    print(X_train.shape)
    X_test = X_test.iloc[:, 0: 148]
    for cls_name, clf in zip(names, classifiers):
        start_time = time.time()
        ...
        for i in range(len(subject)):
            if y_test[i] != clf.predict(predict_result[i]):
                print(subject[i])
                print(y_test[i])
                print(predict_result[i])
                print(predict_cover[i])
        ...
        result = result.tolist()
        if not os.path.isfile('{}/Predict.xlsx'.format(classifier_dir)):
            wb = Workbook()
        else:
            wb = load_workbook('{}/Predict.xlsx'.format(classifier_dir))
        ws = wb.create_sheet(cls_name)
        for row in result:
            ws.append(row)
        wb.save('{}/Predict.xlsx'.format(classifier_dir))
