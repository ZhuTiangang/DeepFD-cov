...
def summary(parent_dir, is_subject, max_iter=5):
    ...
    for model_dir in os.listdir(parent_dir):
        ...
        for mutant_dir in os.listdir(os.path.join(parent_dir, model_dir)):
            ...
            for iter_num in range(1, max_iter + 1, 1):
                ...
                coverage_dir = os.path.join(parent_dir, model_dir, mutant_dir, "iter_{}".format(iter_num), "result_dir",
                ...
                if not os.path.isfile(feature_dir) or not os.path.isfile(coverage_dir):
                    print('skip')
                    continue
                # validate file exist
                ...
                for ftype, fvalue in zip(FAULT_TYPE, fault_list):
                    ...
                    # for debug
                    # print("dataset_summary_dict[model_dir][mutant_dir][iter_num][lb_{}] = {}\n".format(ftype,
                    #                                                                                 label_dict[ftype]))
                ...
                with open(log_dir, "r") as f:
                    ...
                    print(lines)
                    heads = lines[0].strip('\n').split(',')
                    values_last_line = lines[-2].strip("\n").split(",")
                ...
                df_c = pd.read_excel(coverage_dir, sheet_name=1, header=None, index_col=0)
                df_c = pd.DataFrame(df_c.values.T, columns=df_c.index)
                # print(df_c)

                df = df.fillna(0.0)  # 填充缺失值为0
                ...
                coverage_dict = extract_feature(df_c)
                # print(coverage_dict)
                feature_dict = extract_feature(df)
                # print(feature_dict)
                feature_dict = dict(feature_dict, **coverage_dict)
                print(feature_dict)
        ...
        print("  - Current is_killed_cnt: {}, is_faulty_cnt: {}\n".format(is_killed_cnt, is_faulty_cnt))
